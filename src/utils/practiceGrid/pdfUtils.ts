  ...
  try {
    const docDefinition = {
      content: [
        ...
        {
          text: `Total Items: ${group.rowCount} | Total Qty: ${group.totalQty} | Total Amount: $${group.totalAmount.toFixed(2)}`,
          style: "summary"
        },
        { text: "\n" },
          ...
          table: {
            ...
            body: [
              ...
              [
                ...
                { text: '', style: 'totalCell' },
                { text: `$${group.totalAmount.toFixed(2)}`, style: 'totalCell' }
              ]
            ]
          },
          layout: {
    ...
    pdfMake.createPdf(docDefinition as any).download(`Group_${groupIndex + 1}_${group.groupId}.pdf`);
    return { success: true };
  } catch (error) {
...
export const generateAllGroupsPDF = (groupedData: GroupData[]) => {
  try {
    ...
    const grandTotalQty = groupedData.reduce((sum, group) => sum + group.totalQty, 0);
    const grandTotalAmount = groupedData.reduce((sum, group) => sum + group.totalAmount, 0);
    ...
    const content: any[] = [
      ...
      {
        text: `Total Groups: ${totalGroups} | Grand Total Qty: ${grandTotalQty} | Grand Total Amount: $${grandTotalAmount.toFixed(2)}`,
        style: 'grandSummary'
      },
      { text: "\n" }
    ];
    ...
    groupedData.forEach((group, index) => {
      content.push(
        ...
        {
          text: `Items: ${group.rowCount} | Qty: ${group.totalQty} | Amount: $${group.totalAmount.toFixed(2)}`,
          style: 'groupSummary'
        },
        ...
        // Group Table
        {
          ...
          layout: {
            fillColor: function (rowIndex: number) {
              return rowIndex === 0 ? '#4472C4' : (rowIndex % 2 === 0 ? '#F8F9FA' : null);
            }
          },
          margin: [0, 0, 0, 15]
        }
    ...
    pdfMake.createPdf(docDefinition as any).download(`All_Groups_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    return { success: true };
  } catch (error) {
    console.error("❌ PDF Generation Error:", error);
    return { success: false, error };
...
export const generateGroupsSummaryPDF = (groupedData: GroupData[]) => {
  try {
    ...
    pdfMake.createPdf(docDefinition as any).download(`Groups_Summary_${new Date().toISOString().split('T')[0]}.pdf`);
    return { success: true };
  } catch (error) {
    console.error("❌ PDF Generation Error:", error);
    return { success: false, error };
